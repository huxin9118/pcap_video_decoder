...
JNIEXPORT jint JNICALL Java_com_example_pcapdecoder_activity_MainActivity_parsePktInfo(JNIEnv *env, jobject thiz, jstring pcap_jstr, jobject object_list, jobject object_videoInfo){
	...
	if(pcap_file_header->magic == 0xA1B2C3D4){//pcap文件头标识
		...
		return pcapng_parsePktInfo(env, thiz, pcap_jstr, object_list, object_videoInfo);
	}	
...
int pcapng_parsePktInfo(JNIEnv *env, jobject thiz, jstring pcap_jstr, jobject object_list, jobject object_videoInfo){
	LOGI("pcapng_parsePktInfo start");
	jclass class_list = (*env)->FindClass(env,"java/util/ArrayList");
	jmethodID methodID_list_add = (*env)->GetMethodID(env,class_list,"add","(Ljava/lang/Object;)Z");
	...
	// PCAP_FILE_HDR *pcap_file_header;
	// PCAP_PKT_HDR *pcap_pkt_hdr;
	PCAPNG_SECTION_HDR *pcapng_section_header;
	PCAPNG_INTERFACE_DESC *pcapng_interface_description;
	PCAPNG_ENHANCED_PACKET *pcapng_enhanced_packet;
	PCAPNG_SIMPLE_PACKET *pcapng_simple_packet;
	PCAPNG_PACKET *pcapng_packet
	...
	// //偏移PCAP文件头
	// process += sizeof(PCAP_FILE_HDR);
	pcapng_section_header = (PCAPNG_SECTION_HDR*)malloc(sizeof(PCAPNG_SECTION_HDR));
	fread(pcapng_section_header, 1, sizeof(PCAPNG_SECTION_HDR), filePCAP);
	//偏移PCAP文件头
	process += sizeof(PCAPNG_SECTION_HDR);
	
	LOGI("pcapng_section_header->block_type=%u",pcapng_section_header->block_type);
	int is_pcap_file_header_endian_switch = 0;
	if(pcapng_section_header->byte_order_magic == 0x1A2B3C4D){//pcap文件头标识
		is_pcap_file_header_endian_switch = 0;
	}
	else if(pcapng_section_header->byte_order_magic == 0x4D3C2B1A){
		is_pcap_file_header_endian_switch = 1;
	}
	else{
		LOGE("not pcapng file!!!!");
		return -2;
	}	
	LOGI("is_pcap_file_header_endian_switch=%d",is_pcap_file_header_endian_switch);
	
	pcapng_interface_description = (PCAPNG_INTERFACE_DESC*)malloc(sizeof(PCAPNG_INTERFACE_DESC));
	while(fread(pcapng_interface_description, 1, sizeof(PCAPNG_INTERFACE_DESC), filePCAP)){	
		if(!is_pcap_file_header_endian_switch){
			if(pcapng_interface_description->block_type) == 0x00000001){
				pcap_file_linktype = pcapng_interface_description->linktype;
				break;
			}
		}
		else{
			if(switchUint32(pcapng_interface_description->block_type) == 0x00000001){
				pcap_file_linktype = switchUint32(pcapng_interface_description->linktype);
				break;
			}
		}
	}
	...
	while(1){		//如果文件读到底了，就全解析完。 否则缓冲区剩余字节小于PCAP_BOTTOM_ALERT的时候退出循环
		while( pos - pcap_buffer <  len_read - len_alert )
		{	
			pcapng_enhanced_packet = (PCAPNG_ENHANCED_PACKET *)pos;
			if(!is_pcap_file_header_endian_switch){
				if(pcapng_enhanced_packet->block_type) == 0x00000006){
					pcapng_enhanced_packet = (PCAPNG_ENHANCED_PACKET *)pos;
					len_pkt_left = pcapng_enhanced_packet->captured_len;
					break;
				}
				else if(pcapng_enhanced_packet->block_type) == 0x00000003){
					pcapng_simple_packet = (PCAPNG_SIMPLE_PACKET *)pos;
					len_pkt_left = pcapng_simple_packet->packet_len;
					break;
				}
				else if(pcapng_enhanced_packet->block_type) == 0x00000002){
					pcapng_packet = (PCAPNG_PACKET *)pos;
					len_pkt_left = pcapng_packet->captured_len;
					break;
				}
				else{
					pos += sizeof(pcapng_enhanced_packet->block_total_length);
				}
			}
			else{
				if(switchUint32(pcapng_interface_description->block_type) == 0x00000001){
	...
	fclose(filePCAP);
	free(pcapng_section_header);
	free(pcapng_interface_description);
	free(pcap_buffer);
	LOGI("parsePktInfo end");
	return 0;
}
